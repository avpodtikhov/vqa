...
for i in tqdm(range(len(qns_list))):
    ...
    np.save('/mnt/data/users/apodtikhov/vqa/bert/train2014_qns/'+ str(ques['question_id']) + '.npy', token_vecs_sum)
...
for i in tqdm(range(len(qns_list))):
    ...
    for token in token_embeddings:
        ...
        token_vecs_sum.append(sum_vec.cpu().numpy())
    token_vecs_sum = np.array(token_vecs_sum)
    np.save('/mnt/data/users/apodtikhov/vqa/bert/val2014_qns/'+ str(ques['question_id']) + '.npy', token_vecs_sum)
