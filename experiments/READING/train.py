...
with torch.cuda.device(1):
    ...
    def train(optimizer, model, batchsize=64):
        ...
        losses = []
        for step, (x_batch, y_batch, z_batch, idx, cat, texts_feat, texts) in enumerate(dataloader):
            optimizer.zero_grad()
            ...
            pred = model(img_feat_iter, ques_ix_iter, texts_feat)
            ...
            for i in range(64):
                if pred_argmax[i] >= len(dataset.ans_to_ix):
                    ...
                    ans_ = dataset.ix_to_ans[str(pred_argmax[i])]
                ans.append(ans_)
